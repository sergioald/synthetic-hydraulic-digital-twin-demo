...
def classify_twin_state(df: pd.DataFrame) -> pd.DataFrame:
    ...
    inefficient = (result.get("efficiency_estimate", pd.Series(1.0, index=result.index)) < 0.48).fillna(
        False
    ).to_numpy()
    load_mismatch = (result.get("load_command_error_kn", pd.Series(0, index=result.index)).abs() > 220).fillna(
        False
    ).to_numpy()
    pump_degradation = (
    ...
    transient = (result["vibration_proxy"] > result["vibration_proxy"].quantile(0.985)).fillna(
        False
    ).to_numpy()
    investigate = result.get("combined_anomaly", pd.Series(False, index=result.index)).fillna(False).to_numpy()
    ...
    assign(pump_degradation, "pump_degradation_suspected", "temperature, vibration and efficiency indicators align")
    assign(transient, "transient_response", "short-duration vibration or pressure transient detected")
    assign(load_mismatch, "load_response_anomaly", "load demand and command signal are inconsistent")
    assign(pressure_loss, "pressure_loss_suspected", "pressure delta is low under moderate/high demand")
